...
async function migrate() {
  ...
  //   'driver'    — driver applied themselves (also the safe default for rows
  //                 created before this column existed)
  //   'long_term' — generated by a long-term booking
  //   'admin'     — admin booked the driver manually
  await sql`
    ALTER TABLE applications ADD COLUMN IF NOT EXISTS source TEXT NOT NULL DEFAULT 'driver'
  `
...
export const applicationRepo = {
  async apply(shiftId: number, userId: string, reserve = false, source: 'driver' | 'long_term' | 'admin' = 'driver'): Promise<DbApplication> {
    await sql`INSERT INTO applications (shift_id, user_id, reserve, source) VALUES (${shiftId}, ${userId}, ${reserve ? 1 : 0}, ${source})`
